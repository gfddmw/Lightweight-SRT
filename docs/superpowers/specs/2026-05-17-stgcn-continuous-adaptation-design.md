...
## 1. 目标
将现有的基于孤立词分类 of ST-GCN 模型重构为支持连续手语序列建模的 **CSLT Encoder**。通过保留时序维度、维度对齐和预测头更换，使其能够利用 CSL-Daily 数据集和 CorrNet 教师特征进行蒸馏训练。
...
- **目的**: 
    1. **维度对齐**: 将 ST-GCN 的通道数（如 256）映射到 CorrNet 教师特征的维度（如 1024）。
